  ...
  return (
    <main className="pt-32 pb-24 px-6 bg-[#F1F5F9] min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Processus d'admission en 3 étapes avec GPU CSS animations */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-24">
          {steps.map((step, index) => (
            <div
              key={index}
              className="bg-white p-8 rounded-3xl border-2 border-brand-primary/15 shadow-lg hover:shadow-[0_20px_40px_rgba(15,23,42,0.14)] hover:border-brand-secondary hover:-translate-y-2.5 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] relative group flex flex-col justify-between cursor-pointer"
            >
              <div>
        ...
        {/* Section Modalités d'admission avec cadres GPU CSS */}
        <div className="mb-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {modalites.map((item, index) => (
              <div
                key={index}
                className="bg-white p-8 rounded-3xl border-2 border-brand-primary/15 shadow-lg hover:shadow-[0_25px_50px_rgba(37,99,235,0.15)] hover:border-brand-secondary hover:-translate-y-2.5 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col justify-between group cursor-pointer"
              >
                <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="bg-brand-primary p-12 rounded-[40px] text-white shadow-2xl border-2 border-brand-accent/30 relative overflow-hidden">
            <h3 className="text-3xl font-extrabold mb-8 italic text-brand-accent">Documents requis</h3>
            <ul className="space-y-6">
              {documents.map((doc, index) => (
                <li 
                  key={index} 
                  className="flex items-center space-x-4 border-b border-white/15 pb-4 group cursor-pointer hover:translate-x-2 transition-transform duration-200"
                >
          ...
          <div className="bg-white p-10 rounded-[40px] border-2 border-brand-primary/15 shadow-xl">
            ...
            <div className="space-y-6 mb-10">
              {[
                ...
                <div 
                  key={index} 
                  className="flex items-start space-x-4 group cursor-pointer p-2 rounded-2xl hover:bg-gray-50 hover:translate-x-2 transition-all duration-300"
                >
